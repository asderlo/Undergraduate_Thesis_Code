...
addpath(genpath(path_sm), '-begin');

mpsDir = fullfile(path_sm, '..', 'MPS-main');
if ~isfolder(mpsDir)
    mpsDir = 'd:\matlab\end\MPS-main';
end
if ~isfolder(mpsDir)
    error('请设置 mpsDir 为 MPS-main 根目录（含 FormulationKernels、preprocess）。');
end
addpath(mpsDir);
...
gamma_list_mr = 0.1:0.1:0.9;
eta_stage1_mr = 1;

compareOutDir = fullfile(mpsDir, 'MR_MPS', 'compare_end_results');
...
% MR-MPS 使用两阶段搜索（Stage1: eta=1 搜 alpha+sel_k；Stage2: 搜 eta）
p_best = get_MR_MPS_best_params(dataname{1});
alpha_mps = p_best.alpha_mps;       
sel_kernel_mps = p_best.sel_kernel_mps;

%% KH：优先从缓存读取核集合；若无缓存，则 preprocess 一次并写入缓存
kernelCacheDir = fullfile(mpsDir, 'MR_MPS', 'KH_cache');
...
%% ---------- MR-MPS 参数：两阶段搜索 ----------
use_MR_main_two_stage = true;   % false 则改用 get_MR_MPS_best_params 中参数
addpath(fullfile(mpsDir, 'MR_MPS'));
opts_lx = build_opts_lx();
sel_list_mr = ceil(sel_ratio_list_mr * num_kernel);
sel_list_mr = max(1, min(sel_list_mr, num_kernel));
if use_MR_main_two_stage
    fprintf('MR-MPS 参数：正在运行两阶段搜索（Stage1: eta=1；Stage2: search eta+gamma）（repeat_times=%d，可能较久）...\n', repeat_times_search);
    cfg_full = struct('name','full', 'static_on',true, 'dual_on',true, ...
        'force_all_kernels',false, 'dual_entropy_lambda',opts_lx.dual_entropy_lambda);
    ...
    fprintf('  two-stage 最优: alpha=%.2f, sel_k=%d, eta=%.3g, gamma=%.1f, ACC=%.4f\n', ...
        alpha_lx, sel_kernel_lx, eta_lx, gamma_lx, best_lx.ACC);
    try
    ...
    alpha_lx = p_best.alpha_lx;  sel_kernel_lx = p_best.sel_kernel_lx;
    eta_lx = p_best.eta_lx;
    gamma_lx = opts_lx.dual_dynamic_gamma;
    best_lx = struct('alpha',alpha_lx, 'sel_k',sel_kernel_lx, 'eta',eta_lx, ...
        'gamma',gamma_lx, 'ACC',NaN);
end
...
%% ---------- 6) MR-MPS (直接使用 two-stage 最优参数) ----------
fprintf('跑 MR-MPS（a=%.2f, k=%d, eta=%g, gamma=%.1f，rep=%d）...\n', ...
...
if run_ablation
%% ---------- 7) MR-MPS 消融实验：每个配置独立重搜最优参数 ----------
ablation_configs = {};
ablation_configs{end+1} = struct('name','womr',   'dim_c',dim_c, 'force_all_kernels',false, 'static_on',false, 'dual_on',false, 'dual_entropy_lambda',0);
ablation_configs{end+1} = struct('name','wodual', 'dim_c',dim_c, 'force_all_kernels',false, 'static_on',true,  'dual_on',false, 'dual_entropy_lambda',0);

ablation_results = struct();
for aci = 1:numel(ablation_configs)
    cfg_ab = ablation_configs{aci};
    fprintf('重搜 MR-MPS %s 最优参数...\n', cfg_ab.name);
...
end  % run_ablation：§7 消融

%% ---------- 结果（百分比形式，含 ACC / NMI / ARI / F-score）----------
dataname = char(dataname);
fprintf('数据集 %s\n', dataname);
fprintf('===== MKKM/MKKM-SR/SimpleMKKM/LSWMKC/MPS/MPS-LX（统一使用my_nmi_acc，%d trials mean±std，百分比）=====\n', numTrials);
...
fprintf('MR-MPS (a=%.2f, k=%d, eta=%.2g, gamma=%.1f): ACC=%.2f±%.2f%%,  NMI=%.2f±%.2f%%,  ARI=%.2f±%.2f%%,  F-score=%.2f±%.2f%%,  时间=%.2fs\n', ...
...
if run_ablation
    ...
    for ai = 1:numel(ab_names)
        ...
        fprintf('MR-MPS %s (a=%.2f, k=%d, eta=%.2g, gamma=%.1f): ACC=%.2f±%.2f%%,  NMI=%.2f±%.2f%%,  ARI=%.2f±%.2f%%,  F-score=%.2f±%.2f%%,  时间=%.2fs\n', ...
...
compare_summary.MR_MPS = pack_method_result(lx_mean, lx_std, t_lx);
if run_mkkm_sr
...
save(outMat, 'dataname', 'compare_summary', 'ablation_results', 'best_lx', ...
    ...
    'repeat_mr_eval', 'run_ablation', '-v7.3');
fprintf('比较实验结果已保存: %s\n', outMat);
...
fprintf(fid, 'method,MR_MPS,%.8g,%d,%.8g,%.8g,%.8g,%.8g,%.8g,%.8g,%.8g,%.8g,%.8g,%.8g,%.8g\n', ...
...
opts.dual_dynamic_gamma = 0.3;
opts.dual_dynamic_sigma = 0;
opts.dual_entropy_lambda = 0.0;
opts.dual_entropy_eps = 1e-12;
opts.maxIter = 50;
end
