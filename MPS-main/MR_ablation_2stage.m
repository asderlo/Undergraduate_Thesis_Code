% MR_ablation_2stage.m
% 基于当前 main_lx_end.m 的两阶段搜索结果设计消融实验
%
% 消融项：
%  - full：搜索 alpha, sel_k, eta, lambda，启用自适应图
%  - womr：去掉全部流形项，eta=0，关闭自适应图
%  - wodynamic：保留静态流形项，关闭自适应图，即 Z=S0
%
% 输出：
%  每个数据集写入 MR_MPS/ablation_2stage_results/*.csv 与 *.mat
...
opts_base.adaptive_graph_on = false;
opts_base.adaptive_graph_lambda = lambda_stage1;
opts_base.adaptive_graph_warmup = 0;
...
if run_ablation
    stage1Work = numel(alpha_list) * numel(sel_ratio_list) * repeat_times_search * 3;

    stage2Work = ...
        numel(eta_list_coarse) * numel(lambda_list) * repeat_times_search + ... % full
        1 * repeat_times_search + ...                                           % womr
        numel(eta_list_coarse) * repeat_times_search;                            % wodynamic
else
    stage1Work = numel(alpha_list) * numel(sel_ratio_list) * repeat_times_search;
    stage2Work = numel(eta_list_coarse) * numel(lambda_list) * repeat_times_search;
end
...
for di = 1:numel(dataIndices)
    ...
    for ci = 1:numel(configs)
        ...
        HP_use = HP;
        sel_candidates = sel_list;

        [best, doneWork, lastWbUpdate] = search_best_for_config( ...
            KH, HP_use, num_kernel, cfg.dim_c, num_cluster, gt, ...
            alpha_list, sel_candidates, eta_stage1, eta_list_coarse, ...
            lambda_stage1, lambda_list, opts_base, cfg, repeat_times_search, ...
            wb, doneWork, totalWork, lastWbUpdate, wb_update_every_s, ...
            sprintf('[%d/%d] %s: %s', di, numel(dataIndices), dname, cfg.name));

        fprintf('%s 最优: alpha=%.2f, sel_k=%d, eta=%.3g, lambda=%.3g, ACC=%.4f\n', ...
            cfg.name, best.alpha, best.sel_k, best.eta, best.lambda, best.ACC);

        opts = opts_for_config(opts_base, cfg, best.lambda);
        opts.maxIter = 50;
        ...
        for rep = 1:repeat_times_eval
            ...
            [F, ~] = main_lx_end(KH, HP_use, num_kernel, cfg.dim_c, num_cluster, ...
    ...
    if fid < 0
        ...
        fprintf(fid, ...
            'dataset,config,alpha,sel_k,eta,lambda,adaptive_graph_on,search_ACC,ACC_mean,ACC_std,NMI_mean,NMI_std,ARI_mean,ARI_std,F_mean,F_std\n');
        ...
        for ii = 1:numel(fns)
            ...
            fprintf(fid, ...
                '%s,%s,%.8g,%d,%.8g,%.8g,%d,%.8g,%.8g,%.8g,%.8g,%.8g,%.8g,%.8g,%.8g,%.8g\n', ...
                dname, nm, ...
                rr.best.alpha, ...
                rr.best.sel_k, ...
                rr.best.eta, ...
                rr.best.lambda, ...
                rr.cfg.adaptive_graph_on, ...
                rr.best.ACC, ...
                rr.mean(1), rr.std(1), ...
                rr.mean(2), rr.std(2), ...
                rr.mean(3), rr.std(3), ...
...
function [best, doneWork, lastWbUpdate] = search_best_for_config( ...
    ...
    lambda_stage1, lambda_list, opts_base, cfg, repeat_times, ...
...
opts_stage1 = opts_for_config(opts_base, cfg, lambda_stage1);
...
if cfg.adaptive_graph_on
    lambda_candidates = lambda_list;
else
    lambda_candidates = lambda_stage1;
end
...
for ie = 1:numel(eta_candidates)
    ...
    for il = 1:numel(lambda_candidates)

        lambda_z = lambda_candidates(il);

        opts = opts_for_config(opts_base, cfg, lambda_z);
        ...
        for r = 1:repeat_times
            ...
            [F, ~] = main_lx_end(KH, HP, num_kernel, dim_c, num_cluster, ...
                best1.alpha, best1.sel_k, eta, opts);
            ...
            lastWbUpdate = maybe_update_waitbar( ...
                wb, doneWork, totalWork, lastWbUpdate, wb_update_every_s, ...
                sprintf('%s Stage2 eta=%.3g lambda=%.3g (%d/%d)', ...
                wbPrefix, eta, lambda_z, r, repeat_times));
        end
...
function opts = opts_for_config(opts_base, cfg, lambda_z)

opts = opts_base;
opts.adaptive_graph_on = cfg.adaptive_graph_on;
opts.adaptive_graph_lambda = lambda_z;
